# YouTube_Traking

YouTube 추적 프로그램 프로젝트입니다.

## 현재 상태

이 저장소는 GitHub 동기화와 향후 소스 관리를 위해 준비되었습니다. 현재 프로젝트 폴더에는 아직 실제 프로그램 소스가 없으므로, 기능 설명은 저장소 구조와 개발 기준 중심으로 작성되어 있습니다.

## 저장소 구조

```text
.
|-- README.md
|-- docs/
|   `-- MANUAL.md
`-- .gitignore
```

## 주요 기능

- GitHub 기반 소스 관리
- 프로젝트 문서 관리
- 향후 프로그램 기능 및 사용자 매뉴얼 작성 기준 제공

## 개발 및 배포 기준

- 대상 운영체제: Windows 전용
- 개발 언어: Python
- 화면 프레임워크: PySide6
- 배포 방식: 직원 사용을 위한 Windows 실행파일
- DB 연결 방식:
  - Google Drive 기반 `Config_DB.db` 설정 DB를 사용합니다.
  - DB 연결은 공통 모듈인 `DB_conn.py`를 사용합니다.
- 사원 로그인:
  - 사번/비밀번호 로그인은 공통 모듈인 `employee_login.py`를 사용합니다.
  - PySide6 화면에서는 `LoginDialog`를 사용하고, UI 없는 인증은 `authenticate_employee()`를 사용합니다.

## 문서

- [사용자 매뉴얼](docs/MANUAL.md)

## 개발 메모

완성된 프로그램 소스가 추가되면 이 문서에 아래 내용을 갱신합니다.

- 프로그램 목적
- 설치 및 실행 방법
- 설정값 설명
- 빌드 및 테스트 방법
- 주요 화면과 업무 흐름
- 오류 처리 및 문제 해결 방법

DB 연결 로직은 각 화면이나 기능 모듈에 개별로 작성하지 않습니다. 새 코드는 반드시 공통 `DB_conn.py` 모듈을 통해 DB에 접근해야 하며, 패키징된 Windows 실행파일에서도 동일한 연결 방식이 유지되어야 합니다.

사원 로그인 로직도 각 화면에 개별로 작성하지 않습니다. 로그인 성공 사용자는 `LoginUser(sabun, saname)` 형태로 다루며, 비밀번호나 로그인 조회 결과를 로그로 남기지 않습니다.
